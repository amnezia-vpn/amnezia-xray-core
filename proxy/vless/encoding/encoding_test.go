...
func TestRequestSerialization(t *testing.T) {
	...
	_, actualRequest, actualAddons, _, err := DecodeRequestHeader(false, nil, &buffer, Validator, func(uuid2 uuid.UUID) {})
	common.Must(err)
...
func TestInvalidRequest(t *testing.T) {
	...
	_, _, _, _, err := DecodeRequestHeader(false, nil, &buffer, Validator, func(uuid2 uuid.UUID) {})
	if err == nil {
		t.Error("nil error")
	}
}
