...
unknown VLESS UUID
  ...
  -> Xray routing rule
  -> outbound class
  -> strict socket mark/interface binding
...
it supported atomic policy updates. Likewise, `strictBinding` is an additive
config field that an older core can ignore, so deployment must pin a build that
contains this feature rather than relying only on config acceptance.
...
## System routing and fail-closed binding

`ru-system` sets mark `100` with `strictBinding: true`. Xray selects the
outbound class; Linux interprets the mark:
...
also permit `CAP_NET_RAW`. With `strictBinding` enabled, failure to apply a
configured mark or interface aborts the TCP or UDP dial instead of silently
using the default route.

Strict binding belongs on the terminal system-dialing outbound. It is rejected
with `dialerProxy` or an outbound proxy chain because those paths can bypass
that outbound's socket settings. Browser-dialer transports are rejected too;
XHTTP download connections inherit the primary strict mark/interface.
Conflicting custom socket options cannot overwrite the binding. A strict
outbound must configure at least one of `mark` or `interface`.

Binding a source IP or interface instead of a mark changes the outbound and
host routing configuration, not the authentication or mux flow.

## Example configuration

Validate the client sample on any supported host and the server sample on
Linux. The server intentionally uses Linux `SO_MARK`, so strict validation
rejects it on platforms that cannot provide that guarantee:
